...
public class AbstractAtlasEffect implements Disposable {
    private Info info;
    ...
    public AbstractAtlasEffect(String id, float x, float y, float scale, int delay, boolean loop) {
    ...
    public AbstractAtlasEffect(String id, float x, float y) {
        this(id, x, y, 1.0f, 3, false);
    }


    public AbstractAtlasEffect(String id, float x, float y, boolean loop) {
        this(id, x, y, 1.0f, 3, loop);
    }

    public AbstractAtlasEffect(String id, float x, float y, float scale) {
        this(id, x, y, scale, 3, false);
    }

    public AbstractAtlasEffect(String id, float x, float y, float scale, boolean loop) {
    ...
    private class LayerAnimation {
        ...
        void render(SpriteBatch sb) {
            ...
            if (currFrame != null) {
                ...
                img = AbstractAtlasEffect.this.atlas.findRegion(spriteSheetId + String.format("-%05d", currFrameIndex));

                System.out.println(img.getRegionX());

                if (img.rotate) {
                    sb.draw(img.getTexture(),
                            AbstractAtlasEffect.this.curAnimation.xPosition + AbstractAtlasEffect.this.curAnimation.layerAnimations.get(0).xPosition
                                    + img.getRegionX() * Settings.scale - currFrame.originalX,

                            AbstractAtlasEffect.this.curAnimation.yPosition + AbstractAtlasEffect.this.curAnimation.layerAnimations.get(0).yPosition
                                    + img.getRegionY() * Settings.scale + img.packedWidth * Settings.scale + currFrame.originalY,
                            ...
                            AbstractAtlasEffect.this.flipX, AbstractAtlasEffect.this.flipY
                    );
                } else {
                    sb.draw(img.getTexture(),
                            AbstractAtlasEffect.this.curAnimation.xPosition + AbstractAtlasEffect.this.curAnimation.layerAnimations.get(0).xPosition
                            ...
                            AbstractAtlasEffect.this.curAnimation.yPosition + AbstractAtlasEffect.this.curAnimation.layerAnimations.get(0).yPosition
                                    + img.getRegionY() * Settings.scale + currFrame.originalY,
                            0.0f,
                            0.0f,
                            img.packedWidth,
                            img.packedHeight,
