...
public class Repuuken1Effect extends AbstractGameEffect {
    private float x;
    ...
    private int gifFrame = 4;
    private float Frameduration = 32.0f;
    private String  id = "Repuuken1";
    private boolean  soundPlayed  = false;
    private float  tanX = 0.0f;
    private float  tanY = 0.0f;


    public Repuuken1Effect(float fixX,float fixY,float thisX,float thisY) {
        ...
        this.x = thisX ;//.WIDTH * 0.5F - img.packedWidth / 2.0F +Settings.WIDTH * fixX;
    ...
    public void update() {
        ...
        if(this.gifLength-1 <= 0){
            ...
            time =  (int)timer % (this.gifLength);
                ...
                this.x += (Gdx.graphics.getDeltaTime()* this.playSpeed)* 0.125F * tanX;
                this.y += (Gdx.graphics.getDeltaTime()* this.playSpeed) *0.125F * tanY;

        img = new TextureAtlas.AtlasRegion(ZERO[time],0,0,this.gifWidth,this.gifHeight);


        if(!soundPlayed){
            CardCrawlGame.sound.playA(RingOfDestiny.makeID("Repuuken"), MathUtils.random(0.0F, -0.15F));
            soundPlayed = true;
        }
              }
