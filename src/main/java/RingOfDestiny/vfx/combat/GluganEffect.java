...
public class GluganEffect extends AbstractGameEffect {
    private float x;
    ...
    private int gifFrame = 38;
    private float Frameduration = 20.0f;
    private String  id = "Glugan";
    private boolean  soundPlayed  = false;


    public GluganEffect(float fixX,float fixY) {

        for (int i = 0; i < gifFrame; i++) {
            ZERO[i] = new Texture(String.format(RingOfDestiny.assetPath("img/vfx/")+ id +"/Glugan_" + "%02d.png", new Object[]{Integer.valueOf(i)}));
        }
        ...
        this.y = fixY + img.packedHeight  / 2.0F  + Settings.HEIGHT * 0.08f;//-img.packedHeight / 2.0F +Settings.HEIGHT * fixY;
    ...
    public void update() {
        ...
        if(!soundPlayed && time == 16){
            CardCrawlGame.sound.playA(RingOfDestiny.makeID("Glugan"), MathUtils.random(0.0F, -0.1F));
            soundPlayed = true;
        }
              }
