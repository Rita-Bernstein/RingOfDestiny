...
public class HeavenEffect extends AbstractGameEffect {
    private float x;
    ...
    private int imageWidth = 148;
    private int imageHeight = 235;
    private float gifScale = 4.0f;
    private int gifFrame = 24;
    private float Frameduration = 16.0f;
    private String  id = "Heaven";
    private boolean  soundPlayed  = false;


    public HeavenEffect(float fixX,float fixY) {
    ...
    public void update() {
        ...
        if(!soundPlayed){
            CardCrawlGame.sound.playA(RingOfDestiny.makeID("Heaven"), MathUtils.random(0.0F, -0.15F));
            soundPlayed = true;
        }
    }
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(img, this.x, this.y, img.packedWidth / 2.0f, 0, img.packedWidth, img.packedHeight, this.scale, this.scale, 0.0f);
    }
    public void dispose() {}
}
